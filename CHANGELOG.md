...
All notable changes to ACK are logged here. These same notes are available in-app from the TERMINAL view — type `/info` (or tap the `/INFO` shortcut in the STATUSBOX) and send it.

## [1.0-beta.4] - 2026-09-21
...
- **`/info` in the TERMINAL.** Type `/info` and send it, or tap the `/INFO` shortcut next to the TYPING indicator, to read these patch notes line-by-line right in the app. Shake to stop the readout early.
